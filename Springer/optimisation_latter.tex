...
%Let consider its special case: all the alliances have a size equal to two, and the power of all the heroes is the same.
...
%It was shown in \cite{downey1995fixed} that the problem of fining subgraph with a fixed size and maximum density is NP-complete. 
Thus, we have shown that finding the maximum dense subgraph of $k$ vertices is a special case of the DU problem, so it is reducible to DU problem. Therefore the DU problem is no less difficult than NP hard  problem. Thus the DU problem is NP-hard as well.
